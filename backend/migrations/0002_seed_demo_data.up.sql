-- Realistic-scale demo dataset: 21 employees (15 NV, 3 PC, 3 TC), gates
-- A/B/G/D with solver-service's documented defaults, and a 28-day
-- availability window anchored to CURRENT_DATE so the seed stays usable
-- however long after this migration is first applied.
...
-- Baseline: every employee available both shifts, every day, for 28 days
-- starting today.
...
-- A few realistic self-registered unavailable days (mirrors the "N" marker
-- in the source roster) so capacity/shortages have something to show.
...
-- One employee on a short leave stretch, so target-hours proration and
-- replacement-candidate suggestions have a real case to exercise.
