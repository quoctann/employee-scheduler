...
export function CandidatesPanel() {
  ...
  const shifts = (['sang', 'dem'] as const).filter(
    (candidate) => shiftHours[selectedGate]?.[candidate] !== undefined,
  );
    ...
    <div className="space-y-4">
          ...
          <form onSubmit={handleSearch} className="flex flex-wrap items-end gap-4">
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                ...
                onChange={(e) => setRequiresLead(e.target.checked)}
              />
              Cần trưởng ca
            </label>
            <Button
              type="submit"
              disabled={candidates.isPending || !selectedGate || shifts.length === 0}
            >
      ...
      {result && (
        <Card>
          <CardHeader>
            <CardTitle>
              Ứng viên cho {result.target_slot.gate}-
              {shiftHours[result.target_slot.gate]?.[result.target_slot.shift] ?? '?'} ngày{' '}
              {result.target_slot.date}
                    ...
                    <TableRow key={c.employee_id}>
                      ...
                      <TableCell>
                        <div className="flex flex-wrap gap-1">
                          ...
                          {c.would_create_streak && (
                            <Badge variant="destructive">tạo chuỗi ca liên tiếp</Badge>
                          )}
                        </div>
                      </TableCell>
                    </TableRow>
