    ...
    update.mutate(
      {
        ...
        params: {
          nv: editing.nv,
          lead: editing.lead,
          lead_mandatory_role: editing.leadMandatoryRole,
          shift_hours: editing.shiftHours,
        },
      },
      {
        onSuccess: () => {
          toast.success(
            `Đã cập nhật cấu hình cổng ${editing.gate} - ca ${shiftLabel[editing.shift]}`,
          );
          setEditing(null);
        },
        onError: (error) => toast.error(errorMessage(error)),
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Mã cổng mới sẽ áp dụng cho toàn bộ cấu hình, lịch đã duyệt và lịch sử xếp lịch của cổng
            này.
          </p>
        </CardHeader>
        <CardContent>
          ...
          {configQuery.isError && (
            <p className="text-sm text-destructive">
              Không tải được cấu hình: {errorMessage(configQuery.error)}
            </p>
          )}
          {config && (
            <Table>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Với mỗi cổng và ca trực, hệ thống xếp lịch cần biết cần bao nhiêu nhân viên (NV) và bao
            nhiêu trưởng ca (Lead), và liệu vị trí trưởng ca đó có bắt buộc phải do nhân viên vai
            trò Tổ trưởng (TC) đảm nhiệm hay không. Sửa các giá trị bên dưới để thay đổi yêu cầu
            nhân sự cho lần xếp lịch (Solve) tiếp theo — dùng để demo/test cấu hình trước khi áp
            dụng cho lịch thật.
          </p>
        </CardHeader>
        <CardContent>
                  ...
                  return shiftsForGate.map((shift) => {
                    ...
                    return (
                      <TableRow key={`${gate}-${shift}`}>
                        ...
                        <TableCell>
                          <Badge
                            variant={requirement.lead_mandatory_role ? 'destructive' : 'outline'}
                          >
                            {requirement.lead_mandatory_role ? 'Bắt buộc TC' : 'Không bắt buộc'}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-right">
                          {config.shift_hours[gate]?.[shift] ?? '—'}
                        </TableCell>
                        <TableCell className="text-right">
                          <Button
                            type="button"
          ...
          {editing && (
            <form onSubmit={handleSave} className="grid gap-4">
              <DialogHeader>
                ...
                <DialogDescription>
                  Số NV và số trưởng ca là số lượng tối thiểu cần xếp cho ca này. Bật &quot;Bắt buộc
                  vai trò TC&quot; nếu vị trí trưởng ca chỉ được giao cho nhân viên có vai trò Tổ
                  trưởng (TC), không thể là NV thường đứng thay.
                </DialogDescription>
              </DialogHeader>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid gap-1.5">
                  ...
                  <Input
                    ...
                    value={editing.lead}
                    onChange={(event) =>
                      setEditing({ ...editing, lead: Number(event.target.value) })
                    }
                    required
                  />
                </div>
              ...
              <div className="grid gap-1.5">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setEditing({ ...editing, shiftHours: Number(event.target.value) })
                  }
                  required
                />
              </div>
              <label className="flex items-center gap-2 text-sm">
                <input
                  ...
                  onChange={(event) =>
                    setEditing({ ...editing, leadMandatoryRole: event.target.checked })
                  }
                />
          ...
          {renamingGate && (
            <form onSubmit={handleRename} className="grid gap-4">
              <DialogHeader>
                ...
                <DialogDescription>
                  Mã cổng mới sẽ áp dụng cho toàn bộ cấu hình, lịch đã duyệt và lịch sử xếp lịch của
                  cổng này.
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-1.5">
