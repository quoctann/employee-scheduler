...
export function WeeklyRegistrationView() {
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return employees.filter((e) => e.name.toLowerCase().includes(q) || e.employee_id.toLowerCase().includes(q))
  }, [employees, search])
  ...
  async function applyStatus(employeeId: string, date: string, status: DayStatus) {
    ...
    try {
      if (status === 'leave') {
        ...
        await setAvailability.mutateAsync({ employeeId, params: { date, sang: false, dem: false } })
      } else {
  ...
  return (
    <div className="space-y-4">
      ...
      {!employeesQuery.isLoading && filteredEmployees.length === 0 && (
        <p className="text-sm text-muted-foreground">Không tìm thấy nhân viên nào khớp "{search}".</p>
      )}
    </div>
  )
}
