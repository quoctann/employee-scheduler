...
import type {
  ...
  UpdateGateShiftRequirementParams,
} from './endpoints';
...
export function useRenameGate() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ gateCode, newCode }: { gateCode: string; newCode: string }) =>
      api.renameGate(gateCode, newCode),
    onSuccess: (config: SolverConfig) => {
      queryClient.setQueryData(['config'], config);
    },
  });
...
export function useUpdateEmployee() {
  return useEmployeeMutation(
    ({ employeeId, params }: { employeeId: string; params: UpdateEmployeeParams }) =>
