...
export function updateGateShiftRequirement(
  gateCode: string,
  shiftType: ShiftType,
  params: UpdateGateShiftRequirementParams,
) {
  return putJSON<SolverConfig>(
    `/api/v1/config/gates/${encodeURIComponent(gateCode)}/shifts/${encodeURIComponent(shiftType)}`,
    params,
  );
}

export function renameGate(gateCode: string, newCode: string) {
  return putJSON<SolverConfig>(`/api/v1/config/gates/${encodeURIComponent(gateCode)}/rename`, {
    new_code: newCode,
  });
}
